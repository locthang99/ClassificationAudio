...
INDEX = 0
INPUT_MEL = r"/content/gdrive/MyDrive/KLTN/DatasetMelSpectogram_Split"
OUTPUT_MODEL = ""
OUTPUT_MODEL2 = ""
BEFORE_MODEL = ""
BEFORE_MODEL2 = ""
...
def load_data():
    ...
    print(len(data['mel']))
    i= 0
    while i < len(data['mel']):
      if np.array(data['mel'][i]).shape != (1292,128):
        print(np.array(data['mel'][i]).shape)
        del data['mel'][i]
        del data['labels'][i]
        i-=1
      i+=1
...
def train():
    ...
    if INDEX == 0:
        ...
        modelFCN = FCN()
        optimiser = keras.optimizers.Adam(learning_rate=0.0001)
        modelFCN.compile(optimizer=optimiser,
              loss='sparse_categorical_crossentropy',
              metrics=['accuracy'])
    ...
    history2 = modelFCN.fit(X_train, y_train, validation_data=(X_validation, y_validation), batch_size=32, epochs=30)
    modelFCN.save(OUTPUT_MODEL2)
    # # evaluate model on test set
    test_loss, test_acc = modelRes.evaluate(X_test, y_test, verbose=2)
    test_loss2, test_acc2 = modelFCN.evaluate(X_test, y_test, verbose=2)
    print('\nTest accuracy1:', test_acc)
    print('\nTest accuracy2:', test_acc2)
...
if __name__ == "__main__":
    ...
    OUTPUT_MODEL2 = r"/content/gdrive/MyDrive/KLTN/Model_120k/model_40k_FCN_VN_Mel_" + str(INDEX)+".h5"
    BEFORE_MODEL2 = r"/content/gdrive/MyDrive/KLTN/Model_120k/model_40k_FCN_VN_Mel_" + str(INDEX-1)+".h5"
    train()
    # INDEX = 5
    # while INDEX < 10:
    #    OUTPUT_MODEL = r"/content/gdrive/MyDrive/KLTN/Model_120k/model_40k_Res50_VN_Mel_" + str(INDEX)+".h5"
